...
namespace FlightApplication.Services.BookService
{
    public class BookService : IBookService
    {
        ...
        public async Task<ResponseDTO> BookAsync(BookQueryDTO queryDTO)
        {
            ...
            try
            {
                var partialFlightBook = await _planeRep.FindFlightBooking(queryDTO.UserId, queryDTO.Id);
                FlightBooking flightBook;
                if (partialFlightBook != null)
                {
                    partialFlightBook.PricePaid += queryDTO.Price;

                    partialFlightBook.Status = partialFlightBook.PricePaid >= partialFlightBook.PriceTotal
                       ? "Totally Paid"
                       : "Partially Paid";

                    flightBook = partialFlightBook;
                }
                else
                {
